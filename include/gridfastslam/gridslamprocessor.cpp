...
(double srr, double srt, double str, double stt){
  ...
  GridSlamProcessor::Particle::Particle(const ScanMatcherMap& m):
    ...
    frontierManager.setExecutionInterval(FREQ_FILE_PATH);
  }
  
//  GridSlamProcessor::Particle::Particle(const Particle& p) : map(p.map) {
//	  pose 				= p.pose;
//	  weight			= p.weight;
//	  gweight 			= p.gweight;
...
//  }
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){
	...
	for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){	
	  ...
	  m_matcher.registerScan(it->map, it->pose, plainReading, it->frontierManager);
